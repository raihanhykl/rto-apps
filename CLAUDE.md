...
- `railway.json` startCommand: `prisma migrate deploy → prisma db seed → node dist/index.js`
- Menggunakan **Prisma Migrate** (bukan `db push`) — versioned migrations di `prisma/migrations/`
- Baseline migration `0_init` mencakup seluruh schema awal
- Saat menambah perubahan schema: `cd packages/backend && npx prisma migrate dev --name deskripsi_perubahan`
...
- Lokasi: `packages/frontend/e2e/`
- Config: `packages/frontend/playwright.config.ts`
- Jalankan: `cd packages/frontend && npm run e2e`
- UI mode: `cd packages/frontend && npm run e2e:ui`
- Headed mode: `cd packages/frontend && npm run e2e:headed`
- Auto-start backend + frontend via `webServer` config di Playwright
...
npm run lint:frontend        # ESLint frontend (Next.js rules)
npm run format               # Prettier format semua file
npm run format:check         # Cek format tanpa write

# === Testing ===
cd packages/backend && npm test              # Run all unit tests
cd packages/backend && npx tsc --noEmit      # TypeScript check backend
cd packages/frontend && npx tsc --noEmit     # TypeScript check frontend
cd packages/frontend && npm run e2e          # Run E2E tests (Playwright)
cd packages/frontend && npm run e2e:ui       # E2E dengan UI mode
...
# === Database ===
cd packages/backend && npx prisma migrate dev --name deskripsi  # Buat migration baru
cd packages/backend && npx prisma migrate deploy                # Apply migrations (production)
...
- **CI**: GitHub Actions — triggered on PR to main/staging. Jobs: lint (Prettier + ESLint), test (tsc + Jest + build), e2e (Playwright, only on PR to main).
